...
exports.App = exports.Header = exports.NestedResults = exports.ResultGroup = exports.NestedResult = exports.Spec = exports.StatusIcon = exports.basename = undefined;

var _react = __webpack_require__(1);

var _react2 = _interopRequireDefault(_react);

var _createHierarchy = __webpack_require__(7);

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

var basename = exports.basename = function basename(path) {
  return path.split(path.indexOf('__tests__') < 0 ? '/' : '__tests__').pop();
};

var StatusIcon = exports.StatusIcon = function StatusIcon(_ref) {
...
var Spec = exports.Spec = function Spec(_ref2) {
  ...
  return _react2.default.createElement(
    ...
    _react2.default.createElement(
      'h2',
      null,
      _react2.default.createElement(StatusIcon, { status: result.status }),
      basename(result.name)
    ),
    result.summary && _react2.default.createElement(
      'h3',
      null,
      result.summary
    ),
    _react2.default.createElement(NestedResults, { test: result })
  );
};

var NestedResult = exports.NestedResult = function NestedResult(_ref3) {
  var assertion = _ref3.assertion;

  return _react2.default.createElement(
    'div',
    { className: 'assertion ' + assertion.status },
    _react2.default.createElement(StatusIcon, { status: assertion.status }),
    _react2.default.createElement(
      'span',
      { className: 'assertion-title' },
      assertion.title
    ),
...
var ResultGroup = exports.ResultGroup = function ResultGroup(_ref4) {
  ...
  return _react2.default.createElement(
    ...
    group.tests.map(function (test, i) {
      return _react2.default.createElement(NestedResults, { test: test, key: i });
    })
  );
};

var NestedResults = exports.NestedResults = function NestedResults(_ref5) {
  var test = _ref5.test;

  return test && (test.tests && test.tests.length > 0 ? _react2.default.createElement(ResultGroup, { group: test }) : _react2.default.createElement(NestedResult, { assertion: test })) || null;

  return elements;
};

var Header = exports.Header = function Header(_ref6) {
  var results = _ref6.results;
  return _react2.default.createElement(
    'header',
    null,
    results.numFailedTestSuites !== 'undefined' && _react2.default.createElement(
      'div',
      { title: results.numFailedTestSuites + ' failing suites', className: 'metric failed' },
      results.numFailedTestSuites
    ),
    results.numPassedTestSuites !== 'undefined' && _react2.default.createElement(
      'div',
...
var App = exports.App = function App(_ref7) {
  var results = _ref7.results,
      config = _ref7.config;
  return _react2.default.createElement(
    'div',
    null,
    _react2.default.createElement(Header, { results: results }),
    _react2.default.createElement(
      'div',
      null,
      results.testResults.sort(function (a, b) {
        return a.name === b.name ? 0 : a.name < b.name ? -1 : 1;
      }).map(function (result, i) {
        var root = (0, _createHierarchy.createHierarchy)(result, config).tests[0];
        root.name = result.name;
        root.summary = result.summary;
        root.status = result.status;

        return _react2.default.createElement(Spec, { result: root, key: i });
      })
    )
  );
};
...
var getItemId = exports.getItemId = function getItemId(item, config) {
  return item && (item[config.id] || item);
};

var makeLevel = exports.makeLevel = function makeLevel(item, config) {
  var _ref;

  return _ref = {}, _defineProperty(_ref, config.id, getItemId(item, config)), _defineProperty(_ref, config.targetSet, []), _defineProperty(_ref, config.ids, {}), _ref;
};

var getItemIfExistsInLevel = exports.getItemIfExistsInLevel = function getItemIfExistsInLevel(level, item, config) {
  return level[config.targetSet][level[config.ids][getItemId(item, config)]];
};

var createItemInLevel = exports.createItemInLevel = function createItemInLevel(level, item, config) {
  var newLevel = makeLevel(item, config);
  level[config.targetSet].push(newLevel);
  level[config.ids][getItemId(item, config)] = level[config.targetSet].length - 1;
  return newLevel;
};

var addItemToLevel = exports.addItemToLevel = function addItemToLevel(level, item, config) {
  return getItemIfExistsInLevel(level, item, config) || createItemInLevel(level, item, config);
};

var createHierarchy = exports.createHierarchy = function createHierarchy(data, config) {
  var hierarchy = makeLevel('root', config);

  data[config.sourceSet].forEach(function (item) {
    var current = hierarchy;

    item[config.ancestors].forEach(function (ancestor) {
      return current = addItemToLevel(current, _defineProperty({}, config.id, ancestor), config);
    });
...
var _testReporter = __webpack_require__(5);
